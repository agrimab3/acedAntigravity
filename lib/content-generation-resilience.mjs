...
export function classifyProviderFailure({ message, statusCode = null, retryAfterMs = null }) {
  ...
  if (
    /quota exceeded|daily quota|billing details|free_tier_requests|insufficient_quota|quota has been exceeded|exceeded your current quota/i.test(
      normalizedMessage
    )
  ) {
    return {
  ...
  if (
    statusCode === 429 ||
    /rate limit|rate_limit|too many requests|capacity_exceeded|temporarily unavailable|try again later|retry in|high demand|experiencing high demand|resource_exhausted|service unavailable|temporarily overloaded|overloaded/i.test(
      normalizedMessage
    )
  ) {
    return {
...
export async function executeProviderOperation({
  ...
  const emitEvent = (event) => {
    recordProviderEvent(runState, event);
