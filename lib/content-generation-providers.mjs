...
export function resolveGenerationModel(provider, rawModel, env = process.env) {
  ...
  return env.GEMINI_GENERATION_MODEL || env.GEMINI_MODEL || DEFAULT_GEMINI_MODEL;
}
...
export function buildProviderCandidates({
  ...
  for (const candidate of [
    ...
    ...fallbackProviders.map((provider) => ({
      ...
      model:
        mode === "review"
          ? resolveReviewModel(provider, null, null, env)
          : resolveGenerationModel(provider, null, env),
    })),
  ]) {
